...
func _generate_corner_weights(_subdivition_position, _subdivition_fraction):
	...
	for _relative_corner_position in _marching_cube._RELATIVE_CORNER_COORDS:
		...
		if _corner_posistion.y - offset.y <= 0.0:
			_corner_weight = noise.get_noise_3dv(_corner_posistion - offset)
		else:
			_corner_weight = noise.get_noise_3dv(_corner_posistion - offset) + (_corner_posistion.y - offset.y) / 20
